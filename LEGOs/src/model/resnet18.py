...
class Resnet18_MLP(BasicModel):
    def __init__(self, args):
        ...
        self.resnet18 = preactresnet18(initial_channels=64, num_classes=512, dropout = False,  per_img_std = False, stride=1, use_ad=args.use_nfl, ad_heads=4)

        print('using nfl', args.use_nfl)

        self.resnet18.conv1 = nn.Conv2d(16, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.resnet18.conv1_b = nn.Conv2d(16, 64, kernel_size=7, stride=2, padding=3, bias=False)
    ...
    def forward(self, x, embedding, indicator):
        alpha = 1.0
        #print(x.shape)
        features = self.resnet18(x.view(-1, 16, 80, 80))
